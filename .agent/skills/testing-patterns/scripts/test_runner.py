...
def detect_test_framework(project_path: Path) -> dict:
    ...
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists():
        ...
        pytest_cmd = "pytest"
        # On Windows, check for pytest in .venv
        if (project_path / ".venv" / "Scripts" / "pytest.exe").exists():
            pytest_cmd = str(project_path / ".venv" / "Scripts" / "pytest.exe")
            
        result["cmd"] = [pytest_cmd, "-v"]
        result["coverage_cmd"] = [pytest_cmd, "--cov", "--cov-report=term-missing"]
